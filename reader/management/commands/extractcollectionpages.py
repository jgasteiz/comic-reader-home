...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('file_item_id', metavar='file_item_id', type=int, help='Id of the collection to extract.')
        parser.add_argument('start_page', metavar='start_page', type=int, help='Initial page to be extracted')
        parser.add_argument('last_page', metavar='last_page', type=int, help='Last page to be extracted')
        parser.add_argument('extract_path', metavar='extract_path', help='Path where the pages should be extracted to')
    ...
    def handle(self, *args, **options):
        ...
        def _extract_pages(comic):
            pages = get_extracted_comic_pages(comic, list(range(initial_page_number, last_page_number)))
            for idx, page_path in enumerate(pages):
                ...
                new_dst_file_name = os.path.join(extract_path, '%s-%d-%d.%s' % (
                    slugify(comic.name),
                    comic.pk,
                    idx,
                    file_extension
